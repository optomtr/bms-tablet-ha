...
def validate_home_patch(patch, merge=False):
    ...
    for area, room in rooms.items():
        ...
        if not isinstance(room,dict) or set(room)-{"visible","order","exclude","include","roles","name","floor_name","entity_names","cover_directions","entity_icons"}: raise ValueError("Некорректная комната")
        for key,value in room.items():
            ...
            elif key in ("roles","entity_names","cover_directions","entity_icons"):
